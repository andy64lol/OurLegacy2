...
_SPA_DIST = os.path.join(os.path.dirname(os.path.abspath(__file__)), "static", "dist")


@app.route("/assets/<path:filename>")
def serve_spa_assets(filename):
    """Serve Vite-built JS/CSS assets."""
    return send_from_directory(os.path.join(_SPA_DIST, "assets"), filename)


@app.route("/favicon.ico")
def serve_favicon():
    """Serve favicon from static/dist or fallback."""
    fav = os.path.join(_SPA_DIST, "favicon.ico")
    if os.path.exists(fav):
        return send_from_directory(_SPA_DIST, "favicon.ico")
    fav2 = os.path.join(os.path.dirname(os.path.abspath(__file__)), "static", "favicon.ico")
...
@app.before_request
def _serve_spa():
    """Serve Vue SPA for all non-API, non-asset GET requests."""
    path = request.path
    ...
    if (path.startswith("/api/") or
        ...
        path.startswith("/game_assets/") or
        path.startswith("/socket.io") or
        path == "/ping" or
        path == "/favicon.ico"):
        return None
    spa_path = os.path.join(_SPA_DIST, "index.html")
...
async def _handle_mod_command(sid: str, username: str, raw: str) -> bool:
    ...
    await sio.emit(
        "chat_error",
        {
            "message": "Commands have moved to the console (/admin). Chat commands: /me /mods /help"
        },
        to=sid,
    )
    return True
...
def _get_weapon_combat_effects(player, enemy):
    ...
    if smiting:
        if "undead" in enemy_tags or "demon" in enemy_tags:
            ...
            bonus = 0
            effects.append((0, "Smiting has no effect on holy beings."))
        else:
            bonus = smiting
...
def _api_player_summary(player):
    return {
        ...
        "race": player.get("race"),
        "class": player.get("class", player.get("char_class", "")),
        "char_class": player.get("char_class", player.get("class", "")),
        ...
        "attr_points": player.get("attr_points", 0),
...
def _api_battle_summary():
    ...
    companions = session.get("battle_companions") or []
    return {
        "enemy": {
            "name": enemy.get("name"),
            "hp": enemy.get("hp"),
        ...
        "log": (session.get("battle_log") or [])[-15:],
        "companions": [
            {"key": c.get("id", ""), "name": c.get("name", ""), "hp": c.get("hp", 0), "max_hp": c.get("max_hp", 0)}
            for c in companions
        ] if isinstance(companions, list) else [],
    }
...
def _api_battle_outcome(player, enemy, log):
    ...
    if enemy.get("hp", 1) <= 0:
        ...
        leveled = gain_experience(player, exp)
        _group_contribute(
            exp, gold_gain, f"defeated {enemy.get('name', 'an enemy')} in battle"
        )
        loot_item = None
        loot = enemy.get("loot_table", [])
        if loot and random.random() < min(
            0.80, 0.35 + player.get("attr_discovery", 0.0)
        ):
            loot_item = random.choice(loot)
            player["inventory"].append(loot_item)
            log.append(f"The enemy drops: {loot_item}.")
...
def api_action_explore():
    ...
    if possible_bosses and random.random() < 0.08:
        ...
        if boss_data:
            ...
            session["battle_enemy"] = enemy
            session["battle_log"] = [
                f"{enemy['name']} blocks your path! Boss battle! (HP: {enemy['hp']})"
            ]
            session["battle_player_effects"] = {}
            session["battle_enemy_effects"] = {}
            session["battle_companions"] = _build_battle_companions(player)
    ...
    if roll < 0.08:
        dmg = dice.between(5, max(6, player.get("level", 1) * 3))
        player["hp"] = max(1, player["hp"] - dmg)
        events.append(f"Trap! You take {dmg} damage.")
    elif roll < 0.14:
        mp_r = dice.between(10, 30)
        player["mp"] = min(player["max_mp"], player["mp"] + mp_r)
        events.append(f"Ancient shrine: +{mp_r} MP.")
    elif roll < 0.19:
        xp_b = dice.between(15, 40)
        gain_experience(player, xp_b)
        events.append(f"Worn tome: +{xp_b} EXP.")
    elif roll < 0.23:
        finds = random.choice(
            [["Health Potion"], ["Mana Potion"], ["Health Potion", "Rope"]]
        )
        for i in finds:
            player["inventory"].append(i)
        events.append(f"Supplies found: {', '.join(finds)}.")
    if random.random() < 0.30:
        gold_find = dice.between(5, 20)
        player["gold"] += gold_find
        events.append(f"+{gold_find} gold.")
    if random.random() < 0.40:
        tier = max(1, min(area.get("difficulty", 1), 5))
        mats = MATERIALS_BY_TIER.get(tier, MATERIALS_BY_TIER[1])
        gathered = random.choices(mats, k=dice.roll_1d(3))
        for m in gathered:
            player["inventory"].append(m)
        events.append(f"Materials: {', '.join(gathered)}.")
    roll2 = random.random()
    if roll2 < 0.20:
        h = dice.between(10, 30)
        player["hp"] = min(player["max_hp"], player["hp"] + h)
        events.append(f"Healing herb: +{h} HP.")
    elif roll2 < min(0.60, 0.35 + player.get("attr_discovery", 0.0)):
        player["inventory"].append("Health Potion")
        events.append("Found a Health Potion.")
    save_player(player)
    _autosave()
    return jsonify(
        {
            ...
            "message": " ".join(events)
            if events
            else "You explore the area but find nothing notable.",
        }
    )
...
def api_action_travel():
    ...
    if dest_key not in area.get("connections", []):
        return jsonify(
            {"ok": False, "message": "That destination is not reachable from here."}
        ), 400
    session["current_area"] = dest_key
    dest_area = GAME_DATA["areas"].get(dest_key, {})
    dest_name = dest_area.get("name", dest_key.replace("_", " ").title())
    ...
    return jsonify(
        {
            "ok": True,
            "message": f"You travel to {dest_name}.",
            "area_key": dest_key,
            "area_name": dest_name,
...
def api_action_rest():
    ...
    if cost > 0 and player["gold"] < cost:
        return jsonify(
            {"ok": False, "message": f"You need {cost} gold to rest here."}
        ), 400
    player["gold"] -= cost
    player["hp"] = player["max_hp"]
    player["mp"] = player["max_mp"]
...
def api_action_mine():
    ...
    if random.random() > success_chance:
        ...
        return jsonify(
            {
                "ok": True,
                "message": "You swing your pickaxe but find nothing useful.",
                "found": None,
                "player": _api_player_summary(player),
            }
        )
    ...
    return jsonify(
        {
            "ok": True,
            "message": f"You mine {ore}. (+{mining_xp} mining XP)",
            "found": ore,
            "player": _api_player_summary(player),
        }
    )
...
def api_action_buy():
    player, err = _api_resolve_player("shop:write")
    if err:
        return err
    data = request.get_json(force=True, silent=True) or {}
    ...
    if player["gold"] < price:
        return jsonify(
            {
                "ok": False,
                "message": f"Not enough gold. Need {price}g, have {player['gold']}g.",
            }
        ), 400
    player["gold"] -= price
    player["inventory"].append(item_name)
    ...
    return jsonify(
        {
            "ok": True,
            "message": f"Bought {item_name} for {price}g.",
            "item": item_name,
            "price": price,
            "player": _api_player_summary(player),
        }
...
def api_action_sell():
    ...
    return jsonify(
        {
            "ok": True,
            "message": f"Sold {item_name} for {sell_price}g.",
            "item": item_name,
...
def api_action_equip():
    player, err = _api_resolve_player("inventory:write")
    if err:
        return err
    data = request.get_json(force=True, silent=True) or {}
    item_name = data.get("item", "").strip()
    ...
    save_player(player)
    _autosave()
    return jsonify({"ok": ok, "message": msg, "player": _api_player_summary(player)}), (
        200 if ok else 400
    )
...
def api_action_unequip():
    ...
    if not slot:
        return jsonify(
            {"ok": False, "message": "Slot required (weapon, armor, accessory, etc.)."}
        ), 400
    ok, msg = unequip_item(player, slot)
    save_player(player)
    _autosave()
...
def api_action_use_item():
    ...
    if "health" in lower or ("potion" in lower and "mana" not in lower):
        ...
        r = dice.between(25, 50)
        player["mp"] = min(player["max_mp"], player["mp"] + r)
        player["inventory"].remove(item_name)
        msg = f"You use {item_name}: +{r} MP."
    elif "elixir" in lower or "tears" in lower:
        heal = dice.between(50, 100)
        player["hp"] = min(player["max_hp"], player["hp"] + heal)
        player["inventory"].remove(item_name)
        ...
        return jsonify(
            {"ok": False, "message": f"{item_name} cannot be used directly."}
        ), 400
    save_player(player)
    _autosave()
    ...
    if not stunned:
        if not _check_weapon_accuracy(player, enemy):
            ...
            if enemy_effects.get("weaken", {}).get("turns", 0) > 0:
                eff_def = max(
                    0, eff_def - enemy_effects["weaken"].get("def_reduction", 0)
                )
            if enemy_effects.get("armor_crushed", {}).get("turns", 0) > 0:
                eff_def = max(
                    0, eff_def - enemy_effects["armor_crushed"].get("def_reduction", 0)
                )
            eq_weapon = GAME_DATA["items"].get(
                player.get("equipment", {}).get("weapon", ""), {}
            )
            armor_pen = (
                eq_weapon.get("armor_penetration", 0)
                if isinstance(eq_weapon, dict)
                else 0
            )
            if armor_pen:
                eff_def = int(eff_def * (1 - armor_pen / 100.0))
            ...
            for bonus_dmg, bonus_msg in _get_weapon_combat_effects(player, enemy):
                if bonus_dmg > 0:
                    ...
                    log.append(bonus_msg)
            for msg in _get_weapon_on_hit_procs(player, enemy, enemy_effects):
                log.append(msg)
    if enemy["hp"] <= 0:
        _sync_companion_hp_to_player(player, battle_companions)
        _restore_companion_hp(player)
...
def api_battle_defend():
    ...
    if player["hp"] <= 0 or enemy.get("hp", 1) <= 0:
        return _api_battle_outcome(player, enemy, log)
    log.append("You brace yourself, raising your guard.")
    real_def = player["defense"]
    player["defense"] = real_def * 2
    _enemy_take_turn(enemy, player, player_effects, log, battle_companions)
    ...
    if player["hp"] <= 0:
        ...
        if won:
            player["hp"] = 1
            _restore_companion_hp(player)
            log.append("Your companions saved you!")
        session["battle_player_effects"] = player_effects
        session["battle_enemy_effects"] = enemy_effects
        return _api_battle_outcome(player, enemy, log)
    session["battle_enemy"] = enemy
        ...
        return jsonify(
            {
                "ok": True,
                "outcome": "fled",
                "message": f"You fled from the {enemy.get('name', 'enemy')}.",
                "player": _api_player_summary(player),
            }
        )
    e_dmg = max(1, enemy["attack"] - player["defense"] + dice.between(0, 5))
...
def api_battle_spell():
    ...
    weapon = player.get("equipment", {}).get("weapon")
    available = [
        s["name"] for s in get_available_spells(weapon, items_data, spells_data)
    ]
    if spell_name not in available:
        return jsonify(
            {
                "ok": False,
                "message": "That spell is not available with your current weapon.",
            }
        ), 400
    log = session.get("battle_log", [])
    player_effects = session.get("battle_player_effects") or {}
    ...
    spell_data = spells_data.get(spell_name, {})
    result = cast_spell(
        player, enemy, spell_name, spell_data, spell_data.get("effects_data", {})
    )
    ...
    if player["hp"] <= 0:
        ...
        if won:
            player["hp"] = 1
            _restore_companion_hp(player)
            log.append("Companions saved you!")
        session["battle_player_effects"] = player_effects
        session["battle_enemy_effects"] = enemy_effects
        return _api_battle_outcome(player, enemy, log)
    session["battle_log"] = log
...
def api_battle_use_item():
    ...
    if "health" in lower or ("potion" in lower and "mana" not in lower):
        ...
        player["hp"] = min(player["max_hp"], player["hp"] + heal)
        player["inventory"].remove(item_name)
        log.append(f"You use {item_name}: +{heal} HP.")
        used = True
    elif "mana" in lower:
        r = dice.between(25, 50)
        player["mp"] = min(player["max_mp"], player["mp"] + r)
        player["inventory"].remove(item_name)
        log.append(f"You use {item_name}: +{r} MP.")
        used = True
    if not used:
        return jsonify(
            {"ok": False, "message": f"{item_name} cannot be used in battle."}
        ), 400
    _enemy_take_turn(enemy, player, player_effects, log, battle_companions)
    if player["hp"] <= 0 or enemy.get("hp", 1) <= 0:
        session["battle_player_effects"] = player_effects
...
@app.route("/api/catalog/missions")
@limiter.limit("60 per minute")
def api_catalog_missions():
    missions = GAME_DATA.get("missions") or {}
    return jsonify({"ok": True, "missions": missions})


@app.route("/api/catalog/dungeon_list")
@limiter.limit("60 per minute")
def api_catalog_dungeon_list():
    dungeons_data = GAME_DATA.get("dungeons") or {}
    all_dungeons = dungeons_data.get("dungeons", [])
    visited = list(session.get("visited_areas") or [])
    result = []
    for d in all_dungeons:
        allowed = d.get("allowed_areas", [])
        discovered = any(a in visited for a in allowed)
        diff = d.get("difficulty", [1, 3])
        min_level = max(1, diff[0] * 2) if isinstance(diff, list) else max(1, int(diff) * 2)
        result.append({
            "id": d.get("id"),
            "name": d.get("name", "Unknown"),
            "description": d.get("description", ""),
            "rooms": len(d.get("rooms", [])) or d.get("room_count", 5),
            "min_level": min_level,
            "allowed_areas": allowed,
            "discovered": discovered,
        })
    return jsonify({"ok": True, "dungeons": result})


# ── Missing API action routes ─────────────────────────────────────────────────
...
def api_action_craft():
    ...
    save_player(player)
    _autosave()
    return jsonify({"ok": result["ok"], "message": result["message"], "player": _api_player_summary(player)})
...
def api_action_hire_companion():
    ...
    comp_data = GAME_DATA["companions"].get(comp_id)
    if not comp_data:
        return jsonify({"ok": False, "message": "Unknown companion."}), 400
    companions = player.setdefault("companions", [])
    if len(companions) >= 4:
        return jsonify({"ok": False, "message": "Party is full (max 4)."}), 400
    if any((c.get("id") if isinstance(c, dict) else c) == comp_id for c in companions):
        return jsonify({"ok": False, "message": f"{comp_data.get('name')} is already in your party."}), 400
    base_price = comp_data.get("price", 100)
    discount = min(0.50, player.get("attr_gold_discount", 0.0))
    price = max(1, int(base_price * (1.0 - discount)))
    if player["gold"] < price:
        return jsonify({"ok": False, "message": f"Need {price}g to hire {comp_data.get('name')}."}), 400
    player["gold"] -= price
    for stat_key in ("attack_bonus", "defense_bonus", "speed_bonus"):
        bonus = comp_data.get(stat_key, 0)
        if bonus:
    ...
    for stat_key in ("hp_bonus", "mp_bonus"):
        ...
        if bonus:
            ...
            player[f"max_{stat}"] = player.get(f"max_{stat}", 0) + bonus
    rank = comp_data.get("rank", "common")
    _COMPANION_RANK_HP = {"common": 400, "uncommon": 600, "rare": 900, "epic": 1300, "legendary": 2000}
    comp_max_hp = _COMPANION_RANK_HP.get(rank, 400)
    companions.append({
        "id": comp_id,
        "name": comp_data.get("name", comp_id),
    ...
    player.setdefault("hired_companions", [])
    if comp_id not in player["hired_companions"]:
        player["hired_companions"].append(comp_id)
    save_player(player)
    _autosave()
    return jsonify({"ok": True, "message": f"{comp_data.get('name')} joins your party.", "player": _api_player_summary(player)})
...
def api_action_dismiss_companion():
    ...
    player["companions"] = [c for c in companions if (c.get("id") if isinstance(c, dict) else c) != comp_id]
    hired = player.get("hired_companions", [])
    if comp_id in hired:
        hired.remove(comp_id)
        player["hired_companions"] = hired
    if len(player["companions"]) == before:
        return jsonify({"ok": False, "message": "Companion not found."}), 404
    save_player(player)
    _autosave()
    return jsonify({"ok": True, "message": "Companion dismissed.", "player": _api_player_summary(player)})
...
def api_action_complete_mission():
    ...
    completed = list(session.get("completed_missions") or [])
    if mission_id in completed:
        return jsonify({"ok": False, "message": "Mission already completed."}), 400
    can_complete, reason = check_mission_completable(mission_id, player)
    if not can_complete:
        return jsonify({"ok": False, "message": f"Cannot complete: {reason}"}), 400
    mission = GAME_DATA["missions"].get(mission_id, {})
    completed.append(mission_id)
    session["completed_missions"] = completed
    reward = mission.get("reward", {})
    ...
    save_player(player)
    _autosave()
    msg = f"Quest Complete: {mission.get('name', mission_id)} | +{exp}xp +{gold}g"
    if leveled:
...
def api_action_claim_challenge():
    ...
    ch_def = next((c for c in GAME_DATA.get("weekly_challenges", []) if c.get("id") == ch_id), None)
    if not ch_def:
        return jsonify({"ok": False, "message": "Unknown challenge."}), 404
    ch_prog = player.setdefault("weekly_challenges_progress", {})
    prog = ch_prog.get(ch_id, {})
    if prog.get("claimed"):
        return jsonify({"ok": False, "message": "Already claimed."}), 400
    count = prog.get("count", 0)
    target = ch_def.get("target", 0)
    if count < target:
        return jsonify({"ok": False, "message": f"Not completed ({count}/{target})."}), 400
    exp = ch_def.get("reward_exp", 0)
    gold = ch_def.get("reward_gold", 0)
    player["gold"] += gold
    leveled = gain_experience(player, exp)
    ch_prog.setdefault(ch_id, {})["claimed"] = True
    _record_activity("challenges")
    save_player(player)
    _autosave()
    return jsonify({"ok": True, "message": f"Challenge claimed! +{exp}xp +{gold}g", "player": _api_player_summary(player)})
...
def api_action_quick_heal():
    ...
    if err:
        return err
    inv = player.get("inventory", [])
    potions = [p for p in ["Health Potion", "Elixir", "Tonic", "Salve"] if p in inv]
    if not potions:
        return jsonify({"ok": False, "message": "No healing items available."}), 400
    potion = potions[0]
    inv.remove(potion)
    items_data = GAME_DATA.get("items", {})
    item = items_data.get(potion, {})
    heal = item.get("hp_restore", item.get("heal", 50))
    player["hp"] = min(player["max_hp"], player["hp"] + heal)
    save_player(player)
    _autosave()
    return jsonify({"ok": True, "message": f"Used {potion}: +{heal} HP.", "player": _api_player_summary(player)})
...
def api_action_sort_inventory():
    ...
    if err:
        return err
    player["inventory"] = sorted(player.get("inventory", []))
    save_player(player)
    return jsonify({"ok": True, "message": "Inventory sorted.", "inventory": player["inventory"], "player": _api_player_summary(player)})
...
def api_action_auto_equip():
    ...
    save_player(player)
    _autosave()
    msg = " | ".join(msgs) if msgs else "Nothing to auto-equip."
    return jsonify({"ok": True, "message": msg, "player": _api_player_summary(player)})
...
def api_dungeon_enter():
    ...
    dungeon = next((d for d in all_dungeons if d.get("id") == dungeon_id), None)
    if not dungeon:
        return jsonify({"ok": False, "message": "Unknown dungeon."}), 404
    allowed_areas = dungeon.get("allowed_areas", [])
    current_area_key = session.get("current_area", "starting_village")
    visited = list(session.get("visited_areas") or [])
    if not any(a in visited for a in allowed_areas):
        return jsonify({"ok": False, "message": "You haven't discovered this dungeon yet."}), 403
    if current_area_key not in allowed_areas:
        area_info = GAME_DATA.get("areas", {}).get(allowed_areas[0], {})
        area_name = area_info.get("name") or allowed_areas[0].replace("_", " ").title()
        return jsonify({"ok": False, "message": f"You must travel to {area_name} first."}), 400
    diff = dungeon.get("difficulty", [1, 3])
    min_level = max(1, diff[0] * 2) if isinstance(diff, list) else 1
    if player.get("level", 1) < min_level:
        return jsonify({"ok": False, "message": f"Need level {min_level} to enter."}), 400
    rooms = generate_dungeon_rooms(dungeon, dungeons_data)
    session["active_dungeon"] = {
        "dungeon": dungeon,
        "rooms": rooms,
    ...
    session.modified = True
    save_player(player)
    return jsonify({"ok": True, "message": f"You enter {dungeon.get('name', 'the dungeon')}!", "dungeon_name": dungeon.get("name")})


@app.route("/api/dungeon/room", methods=["GET"])
@limiter.limit("60 per minute")
def api_dungeon_room():
    player = get_player()
    active = session.get("active_dungeon") or {}
    if not player or not active:
        return jsonify({"ok": False, "message": "No active dungeon."}), 404
    rooms = active.get("rooms", [])
    idx = active.get("room_index", 0)
    if idx >= len(rooms):
        return jsonify({"ok": True, "completed": True, "message": "Dungeon complete!"})
    room = rooms[idx]
    dungeon = active.get("dungeon", {})
    current_challenge = active.get("current_challenge")
    room_type = room.get("type", "empty")
    ...
    return jsonify({
        ...
        "room_num": idx + 1,
        "total_rooms": len(rooms),
        "dungeon": {"name": dungeon.get("name"), "id": dungeon.get("id")},
        "current_challenge": current_challenge,
    })


@app.route("/api/dungeon/proceed", methods=["POST"])
@limiter.limit("30 per minute")
def api_dungeon_proceed():
    player, err = _api_resolve_player("game:write")
    if err:
        return err
    active = session.get("active_dungeon") or {}
    ...
    idx = active.get("room_index", 0)
    if idx >= len(rooms):
        return api_dungeon_complete_inner(player)
    room = rooms[idx]
    room_type = room.get("type", "empty")
    dungeon = active.get("dungeon", {})
    dungeons_data = GAME_DATA.get("dungeons", {})
    ...
    if room_type == "battle":
        ...
        if result["type"] == "battle":
            ...
            session["active_dungeon"] = active
            save_player(player)
            return jsonify({"ok": True, "outcome": "battle_started", "message": f"Battle against {enemy.get('name')}!", "battle": _api_battle_summary()})
        ...
        if result["type"] == "ambush":
            enemy = result["enemy"]
            session["battle_enemy"] = enemy
            session["battle_log"] = [f"AMBUSH! A {enemy.get('name', 'enemy')} leaps from the shadows!"]
            session["battle_player_effects"] = {}
            session["battle_enemy_effects"] = {}
            session["battle_companions"] = _build_battle_companions(player)
            active["room_index"] = idx + 1
            ...
            session["active_dungeon"] = active
            save_player(player)
            return jsonify({"ok": True, "outcome": "battle_started", "message": f"Ambush! {enemy.get('name')}!", "battle": _api_battle_summary()})
        msgs = [m["text"] for m in result.get("messages", [])]
    elif room_type == "shrine":
        result = process_shrine_room(player)
        msgs = [m["text"] for m in result.get("messages", [])]
    elif room_type in ("chest", "trap_chest"):
        result = (process_chest_room if room_type == "chest" else process_trap_chest_room)(player, room, dungeons_data, items_data)
        msgs = [m["text"] for m in result.get("messages", [])]
    elif room_type == "empty":
        result = process_empty_room(player)
        msgs = [m["text"] for m in result.get("messages", [])]
    active["room_index"] = idx + 1
    active["current_challenge"] = None
    active["challenge_answered"] = False
    session["active_dungeon"] = active
    save_player(player)
    if active["room_index"] >= len(rooms):
        return api_dungeon_complete_inner(player, msgs)
    next_room = rooms[active["room_index"]]
    return jsonify({"ok": True, "outcome": "room_processed", "messages": msgs, "room": next_room, "room_num": active["room_index"] + 1, "total_rooms": len(rooms), "player": _api_player_summary(player)})


def api_dungeon_complete_inner(player, extra_msgs=None):
    active = session.pop("active_dungeon", {})
    dungeon = active.get("dungeon", {})
    dungeons_data = GAME_DATA.get("dungeons", {})
    result = complete_dungeon(player, dungeon, dungeons_data)
    msgs = (extra_msgs or []) + [m["text"] for m in result.get("messages", [])]
    save_player(player)
    _autosave()
    _record_activity("dungeons")
    return jsonify({"ok": True, "outcome": "dungeon_complete", "messages": msgs, "player": _api_player_summary(player), "message": f"Dungeon complete! {dungeon.get('name', '')}"})


@app.route("/api/dungeon/answer", methods=["POST"])
@limiter.limit("20 per minute")
def api_dungeon_answer():
    player, err = _api_resolve_player("game:write")
    if err:
        return err
    data = request.get_json(force=True, silent=True) or {}
    ...
    dungeons_data = GAME_DATA.get("dungeons", {})
    challenge = active.get("current_challenge", {})
    result = answer_question(player, challenge, answer, dungeons_data)
    active["challenge_answered"] = True
    session["active_dungeon"] = active
    save_player(player)
    return jsonify({"ok": result["ok"], "message": result.get("message", ""), "messages": [m["text"] for m in result.get("messages", [])], "player": _api_player_summary(player)})


@app.route("/api/dungeon/choose", methods=["POST"])
@limiter.limit("20 per minute")
def api_dungeon_choose():
    player, err = _api_resolve_player("game:write")
    if err:
        return err
    data = request.get_json(force=True, silent=True) or {}
    ...
    dungeons_data = GAME_DATA.get("dungeons", {})
    challenge = active.get("current_challenge", {})
    result = answer_multi_choice(player, challenge, choice_idx, dungeons_data)
    active["challenge_answered"] = True
    session["active_dungeon"] = active
    save_player(player)
    return jsonify({"ok": result.get("ok", True), "message": result.get("message", ""), "messages": [m["text"] for m in result.get("messages", [])], "player": _api_player_summary(player)})


@app.route("/api/dungeon/complete", methods=["POST", "GET"])
@limiter.limit("10 per minute")
def api_dungeon_complete():
    player, err = _api_resolve_player("game:write")
    if err:
        return err
    return api_dungeon_complete_inner(player)
...
@limiter.limit("10 per minute")
def api_dungeon_abandon():
    session.pop("active_dungeon", None)
    session.modified = True
    return jsonify({"ok": True, "message": "Dungeon abandoned."})
...
def api_market_buy():
    ...
    market_api = get_market_api()
    listing = next((l for l in market_api.get_listings() if l.get("item_name") == item_name), None)
    if not listing:
        return jsonify({"ok": False, "message": "Item not found in market."}), 404
    price = listing.get("price", 0)
    if player["gold"] < price:
        return jsonify({"ok": False, "message": f"Need {price}g (have {player['gold']}g)."}), 400
    player["gold"] -= price
    player["inventory"].append(item_name)
    save_player(player)
    _autosave()
    return jsonify({"ok": True, "message": f"Bought {item_name} for {price}g.", "player": _api_player_summary(player)})


@app.route("/api/action/market/reset", methods=["POST"])
@limiter.limit("5 per minute")
def api_market_reset():
    market_api = get_market_api()
    market_api.reset()
    return jsonify({"ok": True, "message": "Market refreshed."})
