...
def get_available_spells(weapon_name: Optional[str], items_data: Dict[str,
                                                                      Any],
...
def can_cast_spell(player: Dict[str, Any],
    ...
    if player.get('mp', 0) < cost:
        return {
            'ok': False,
            'message':
            f'Not enough MP! Need {cost}, have {player.get("mp", 0)}.'
        }
    return {'ok': True}
...
def cast_spell(player: Dict[str, Any], enemy_dict: Dict[str, Any],
    ...
    if spell_type == 'damage':
        ...
        if roll == 1:
            messages.append({
                'text': 'Critical miss! The spell fizzles!',
                'color': 'var(--red)'
            })
        elif roll == 20:
            messages.append({
        ...
        for effect_name in spell_data.get('effects', []):
            ...
            elif etype == 'stun' and random.random() < effect.get(
                    'chance', 0.5):
                messages.append({
        ...
        for effect_name in spell_data.get('effects', []):
            ...
            for k, v in effect.items():
                if isinstance(v,
                              (int, float)) and (k.endswith('_bonus')
                                                 or k in ('absorb_amount', )):
                    modifiers[k] = int(v)
            ...
            mod_str = ', '.join(
                f"+{v} {k}"
                for k, v in modifiers.items()) if modifiers else effect.get(
                    'type', '')
            messages.append({
                'text':
                f'Buff applied: {effect_name} ({mod_str}) for {duration} turns!',
                'color': 'var(--green-bright)'
            })
        ...
        for effect_name in spell_data.get('effects', []):
            effect = effects_data.get(effect_name, {})
            etype = effect.get('type', '')
            if etype == 'action_block' and random.random() < effect.get(
                    'chance', 0.5):
                messages.append({
                    'text': f'{enemy_dict.get("name", "Enemy")} is stunned!',
                    'color': 'var(--yellow)'
