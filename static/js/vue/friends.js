...
createApp({
    ...
    methods: {
        async selectFriend(username) {
            ...
            if (!this.dmMessages[username]) {
                ...
                try {
                    ...
                    this.toast('Failed to load messages');
                }
            }
        ...
        async sendDm() {
            ...
            try {
                const res = await fetch("/api/dm/send", {
                    ...
                    body: JSON.stringify({
                        recipient: this.activeFriend,
                        message: text,
                    }),
                });
                const data = await res.json();
                if (data.ok) {
                    if (!this.dmMessages[this.activeFriend])
                        this.dmMessages[this.activeFriend] = [];
                    this.dmMessages[this.activeFriend].push({
        ...
        sendTradeInvite() {
            if (this.socket)
                this.socket.emit("trade_request", { to: this.activeFriend });
            this.tradeState = "waiting";
        },
        acceptTrade() {
            if (this.socket && this.pendingTradeId) {
                this.socket.emit("trade_accept", {
                    trade_id: this.pendingTradeId,
                });
        ...
        declineTrade() {
            if (this.socket && this.pendingTradeId) {
                this.socket.emit("trade_decline", {
                    trade_id: this.pendingTradeId,
                });
            }
        ...
        cancelTrade() {
            if (this.socket)
                this.socket.emit("trade_cancel", { to: this.activeFriend });
            this.showTrade = false;
            this.pendingTradeId = null;
        },
        confirmTrade() {
        ...
        connectSocket() {
            ...
            socket.on("friend_request", (data) => {
                const alreadyHave = this.requests.some(
                    (r) => r.from_user === data.from,
                );
                if (!alreadyHave) {
                    this.requests.push({
